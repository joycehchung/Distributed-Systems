   ...
   // public static boolean remulticast = false;
	...
	public void SetupGui() {
        ...
        // 	Node Information Pane
	        ...
	        myInfoLabel.setText("<html>My Information:<br>" + 
	        		"<UL><LI> NAME: " + nodeME.name + 
	        		"<LI> IP: " + nodeME.ip + 
	        		"<LI> PORT: " + nodeME.port + "</UL></html>");
    ...
    public TimeStampedMessage ProcessMessage() {
        ...
        if (!nodeME.receiveQueue.isEmpty()) {
            try {
        		...
        		// If this is a multicast message received
            	} else {
            		for (int i=0; i<myGroups.size(); i++) {
            	...
            	if (msg.get_kind().equals("REQUEST")) {
            		...
            		// Update timestamp of global logical clock
            		...
            		if (msg.get_source().equals(nodeME.name)) {
						// Report a request successfully made and reset number of replies
                    	numRequests++;
                    	//numReplies = 0;
	        	        requestLabel.setText("<html><UL><LI>Number of Requests Made: " + numRequests + "<LI>" + "   Number of Replies Received: " + numReplies + "</UL></html>");
	                    mainFrame.repaint();
					...
					// Send reply message
						...
						if (msg.get_source().equals(nodeME.name)) {
    	                    // Send myself a reply message
		    				TimeStampedMessage myReply = new TimeStampedMessage(msg.get_source(),"REPLY","");
		    				myReply.set_source(nodeME.name);
		    				myReply.set_seqNum(sequenceNumber);
		    				sequenceNumber++;
		    				myReply.set_nodeIndex(GetSendNodeIndex(msg.get_source()));
		    				myReply.set_globalClock(Integer.parseInt(clockService.get_sendTimeStamp().ts));
		    				myReply.set_timeStamp(clockService.get_sendTimeStamp());
		    				nodeME.receiveQueue.add(myReply);
						} else {
					...
					if (!nodeME.requestQueue.isEmpty()) {
						...
						try {
							...
							if (request_msg.get_source().equals(nodeME.name)) {
	    	                    // Send myself a reply message
			    				TimeStampedMessage myReply = new TimeStampedMessage(request_msg.get_source(),"REPLY","");
			    				myReply.set_source(nodeME.name);
			    				myReply.set_seqNum(sequenceNumber);
			    				sequenceNumber++;
			    				myReply.set_nodeIndex(GetSendNodeIndex(request_msg.get_source()));
			    				myReply.set_globalClock(Integer.parseInt(clockService.get_sendTimeStamp().ts));
			    				myReply.set_timeStamp(clockService.get_sendTimeStamp());
			    				nodeME.receiveQueue.add(myReply);
							} else {
